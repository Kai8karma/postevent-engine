...
|---|---|---|---|
| **M1 Lead List Enrichment** — dedupe/fuzzy-match against HubSpot, infer title/function/seniority/size/industry, enrich company data via Clay, score ICP fit; HubSpot-ready file >90% complete with tier, region, owner, lifecycle | [`modules/m1-enrichment/enrich.py`](modules/m1-enrichment/enrich.py) (live default): HubSpot Search dedupe → LLM gray-zone adjudication → LLM row inference + firmographics + ICP tier with rationale (rules validate) → `--clay-results` merge with per-field `*_source` → [`push_to_hubspot.py`](modules/m1-enrichment/push_to_hubspot.py) upsert. n8n: [`m1-lead-enrichment.json`](orchestrator/n8n/railway/m1-lead-enrichment.json) | `<out>/hubspot_ready.csv`, `hubspot_contacts.csv`, `hubspot_companies.csv`, `quality_report.json`, `dedupe_report.json`, `live_inference_report.json`. The committed copies from the slice below are openable: [`hubspot_ready.csv`](out/receipts/m1-live-slice-30/outputs/hubspot_ready.csv), [`hubspot_contacts.csv`](out/receipts/m1-live-slice-30/outputs/hubspot_contacts.csv), [`hubspot_companies.csv`](out/receipts/m1-live-slice-30/outputs/hubspot_companies.csv), [`enriched.json`](out/receipts/m1-live-slice-30/outputs/enriched.json), [`dedupe_report.json`](out/receipts/m1-live-slice-30/outputs/dedupe_report.json), [`live_inference_report.json`](out/receipts/m1-live-slice-30/outputs/live_inference_report.json) | [`out/receipts/m1-live-slice-30/`](out/receipts/m1-live-slice-30/) — 30-row live slice: **96.2% contact / 93.0% company verified completeness, PASS** at the 90% bar, 10 LLM calls attempted, 7 parsed cleanly, live HubSpot search. **In this slice the model scored no ICP tiers**: all three ICP batches came back unusable (`icp_parse_failures: 3`, `icp_rows_scored_by_llm: 0`) and all 32 rows took their tier from the rule table (`icp_source: {rules: 32}`). The model's contribution to *this* receipt was dedupe adjudication and firmographic inference. Push: [`out/receipts/m1-push-slice30-2026-09-16.json`](out/receipts/m1-push-slice30-2026-09-16.json) — 32 contacts, 23 companies, 27 associations **as reported by the upsert calls**. That receipt's `verify` block reads `"status": "not_run"`, so these are the write's own counts, not an independent read-back |
...
- **M3 extraction → per-asset generation → grounding gate.** One extraction pass, normalised against real transcript
  ...
  committed run that came to 39 checks over `youtube.md`, `infographic.md` and `social.md`; `grounding_report.json`
  records `claims_checked: 0` for `blog.md`, so the blog draft is not covered by that 39/39.
...
- **M4's engagement stream is seeded.** Opens, clicks, page views, form fills and lifecycle changes did not happen on
  ...
  push and 7 by the seed phase — `dashboard_data.json`'s 14-day window dates them `{"2026-09-13": 1,
  "2026-09-21": 39}`, i.e. 39 of the 40 share a single day and one is older — and the dashboard card and the
  narrative's first sentence say so.
...
- M1 — 30-row live slice: 96.2% contact / 93.0% company verified completeness, PASS against the brief's 90% bar;
  ...
  work** — all three ICP batches came back unusable and all 32 tiers came from the rule table
  (`icp_rows_scored_by_llm: 0`); the model's verified contribution here is the one gray-zone dedupe adjudication
  and the firmographic inference (29 of 32 industries, 32 of 32 employee counts). Push of 32 contacts / 23 companies
...
- M3 — blog 1106 words inside the 800–1200 gate, 18 chapters, 8 social posts across both platforms; grounding 39/39
  over `youtube.md`, `infographic.md` and `social.md` (`blog.md` contributed 0 claims to that report, so it is not
  covered by the 39/39); 3 clips cut in both ratios — the SRT captions and thumbnails are in the package, the six
  MP4s are not and are regenerated by `make_clips.py`.
